...
> 🔀 **权威边界**：本 Demo 里,**实心物体（树、石）已收归服务端权威生成并下发**（因为它们要参与碰撞，见[第一部分 · 6](#6-空间碰撞检测障碍物)）；客户端 `srand` 只负责生成**不参与碰撞的纯装饰物**（灌木、花）。区分标准很简单——**会挡路的归服务端，纯好看的留客户端**。
...
│   ├── AOI.ts             # 九宫格算法实现
│   ├── Obstacle.ts        # 障碍物：确定性生成 + 空间网格 broad-phase
...
5. ~~**地图障碍物**：增加碰撞检测，理解空间分割数据结构~~ ✅ 已实现，见 [第一部分 · 6. 空间碰撞检测](#6-空间碰撞检测障碍物)
